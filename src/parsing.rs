...
pub fn parse_bible(text: &str) -> Result<Bible, String> 
{
    ...
    for (_, [book_name, chapter_name, _verse_name, text]) in pattern.captures_iter(&rest).map(|c| c.extract())
    {
        let chapter_number = chapter_name.parse().unwrap();

        if books.is_empty() || books.last().unwrap().name != book_name {
            books.push(Book { name: book_name.into(), chapters: vec![] });
        }
    ...
    Ok(Bible { name: version.into(), desc: description.into(), books })
}
